 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 0; t < N - 2; t++) {
      // TODO: Is 500 a good value here?
      fg[0] += 800 * CppAD::pow(vars[delta_start + t + 1]
    ...
    // Initial constraints.
    // Add 1 to each of the starting indices due to cost being at fg[0].
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      fg[1 + cte_start + t] =
            cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi1) * dt));
      fg[1 + epsi_start + t] =
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // 4 * 10 + 2 * 9
  // N = num_timesteps
  // There is one less actuation than timesteps since the first is our current
  // position.
  // N_actuations = N - 1
  // State is a 6 element vector in this setup.
  // N_state = 6
  // n_vars = N * N_state + N_actuations * 2

  size_t n_vars = N * 6 + (N - 1) * 2;
  // TODO: Set the number of constraints
  // Number of constraints is N * N_state
  ...
  // Set all non-actuators upper and lower limits to max negative
  ...
  // TODO: Possibly change these values.
  for (i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.435332;
